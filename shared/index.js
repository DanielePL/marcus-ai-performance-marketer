...
// 🔥 FIXED: GoogleAdsService Import Issue
...
// 🔥 FIXED: Import Services correctly
const livePerformanceService = require('./services/livePerformanceService');

// 🔥 FIXED: GoogleAdsService is exported as SINGLETON INSTANCE, not class
const googleAdsService = require('./services/integrations/googleAdsService');
// ↑ NICHT: new GoogleAdsService() - es ist schon eine Instanz!
...
// API Rate Limiting für AI Endpoints
...
app.get('/api/status', async (req, res) => {
  try {
    ...
    // 🔥 FIXED: Test Google Ads API Connection using singleton instance
    let googleAdsStatus = { status: 'not_configured' };
    if (process.env.GOOGLE_ADS_DEVELOPER_TOKEN) {
      try {
        console.log('🔌 Testing Google Ads API connection...');
        // 🔥 FIXED: Use singleton instance directly (no 'new')
...
const startServer = async () => {
  try {
    ...
    // Connect to Database using Marcus Database Config
...
// 🔥 FIXED: Graceful Shutdown (without livePerformanceService.stop which doesn't exist)
process.on('SIGTERM', () => {
